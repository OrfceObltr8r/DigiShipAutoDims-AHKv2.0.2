...
save.Add("Button", "Default", "Batch Labels").OnEvent("Click", BatchLabels)             ;Add button to save user input line by line and then trigger automation loop
...
BatchLabels(*){
    ...
    if (UserText != ""){ ;Check if user text is NOT empty
        ...
        lines := StrSplit(UserText, "`n", "`r") ;String Split user input (`n splits by line)
...
;===========================================================================================================================================
                ;anything after this line is what will happen after within the loop
    sleep 200
    CoordMode "Pixel" , "Screen"
    ScreenClear := False
    While (!ScreenClear) { ;check to make sure there is no input on screen 
    ...
    If PixelSearch(&Px, &Py, 910, 228, 910, 228, 0xFF0000) ;detected Order Status Issue, skip to next
        { ;If issue, reset and skip to next order
            ...
            MsgBox("Skipping to next order",,"T1")
            WinActivate ("Shipments")
            Coordmode "Mouse", "Screen"
            ...
            If PixelSearch(&PX, &Py, 468, 561, 468, 561, 0xFFA500) 
                ...
                MsgBox("Special Instruction Detected! `n Please Resolve Conflict then press the Pause key on your keyboard",,"T3")
                Pause
                Sleep 500
                WinActivate("Shipments")
                sleep 100
